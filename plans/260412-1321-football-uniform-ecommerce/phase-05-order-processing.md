...
# Phase 5: Order Processing + Email + Excel Generation

## Context
- Depends on Phase 3+4 (custom order data)
...
- Email to shop: order info + Excel + logo files + mockup screenshots + print config
...
Checkout Submit → POST /api/orders
  ...
  │   └── Generate mockup image (Canvas on server) → upload to Cloudinary
...
- Attachments: Excel file, logo files (from Cloudinary URLs), mockup images
...
- [ ] Order creation in DB (transaction)
